...
| Disgust Augmented* | 13,315 | 0 | 0 | 0 | 0 | 0 | 0 | 13,315 |
| **Total after cleaning****| **473,250** | **143,067** | **121,187** | **59,137** | **49,649** | **57,356** | **22,640** |  **20,034** |
| **Prevalence percentage**** |   | **30.23%** | **25.61%** | **12.53%** | **12.12%** | **10.49%** | **4.78%** |  **4.23%** |

*The 13k augmented disgust instances come from the scripts `disgust_paraphrase.py` and `disgust_backtranslate.py` that created new instances based on the original 6.7k collected from the last 3 linked datasets
...
2. Run the notebook `merge_love_surprise_disgust.ipynb`
    * This will produce ***love_surprise_bonus.csv*** and ***disgust_original.csv***
3. After that, you will run the script `disgust_paraphrase.py`

    ```
    python -m scripts.disgust_paraphrase
    ```
    * This script will produce file ***data/raw/disgust_paraphrased.csv***
    * It's important that running this script is finished **before running the next step**.
4. Run the script `disgust_backtranslate.py` from the root directory
    
    ```
    python -m scripts.disgust_backtranslate --model_type m2m100
    ```
   * `disgust_backtranslate.py` requires either a bigger GPU or to run on CPU but it will be slower (***Note**: Running on CPU will require changing the code and possibly adding freeze support aside from decreasing the batch size, but I managed to run it on NVIDIA L40S GPU in 15 minutes.*)
   * This script will produce file ***data/raw/disgust_backtranslated.csv***.
5. Run `merge_to_6labels_7labels.py` to produce:
    * ***data/raw/dataset_6labels_more.csv***, and/or
    * ***data/raw/dataset_7labels_ids.csv***
6. The raw datasets of 6/7 emotions are ready.



## Label prevalence in the merged (7-label) dataset

| Emotion Label | Count   | Percentage | *Cumulative* |
|---------------|---------|------------|--------------|
| Joy           | 143,067 | 30.2307%   | *30.23%*     |
| Sadness       | 121,187 | 25.6074%   | *55.84%*     |
| Anger         | 59,317  | 12.5340%   | *68.37%*     |
| Love          | 57,356  | 12.1196%   | *80.49%*     |
| Fear          | 49,649  | 10.4911%   | *90.98%*     |
| Surprise      | 22,640  | 4.7839%    | *95.77%*     |
| Disgust       | 20,034  | 4.2333%    | *100.00%*    |
| **Total**     |**473,250**| **100.0000%**|              |
